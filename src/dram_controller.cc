...
void
MEMORY_CONTROLLER::reset_remain_requests(
    PACKET_QUEUE* queue, uint32_t channel) {
...
void
MEMORY_CONTROLLER::schedule(PACKET_QUEUE* queue) {
  uint64_t read_addr;
...
void
MEMORY_CONTROLLER::process(PACKET_QUEUE* queue) {
...
int
MEMORY_CONTROLLER::add_rq(PACKET* packet) {
...
int
MEMORY_CONTROLLER::add_wq(PACKET* packet) {
...
int
MEMORY_CONTROLLER::add_pq(PACKET* packet) {
  return -1;
}

void
MEMORY_CONTROLLER::return_data(PACKET* packet) {}

void
MEMORY_CONTROLLER::update_schedule_cycle(PACKET_QUEUE* queue) {
...
void
MEMORY_CONTROLLER::update_process_cycle(PACKET_QUEUE* queue) {
...
int
MEMORY_CONTROLLER::check_dram_queue(PACKET_QUEUE* queue, PACKET* packet) {
  // search write queue
...
uint32_t
MEMORY_CONTROLLER::dram_get_channel(uint64_t address) {
...
uint32_t
MEMORY_CONTROLLER::dram_get_bank(uint64_t address) {
...
uint32_t
MEMORY_CONTROLLER::dram_get_column(uint64_t address) {
...
uint32_t
MEMORY_CONTROLLER::dram_get_rank(uint64_t address) {
...
uint32_t
MEMORY_CONTROLLER::dram_get_row(uint64_t address) {
  if (LOG2_DRAM_ROWS == 0)
    return 0;
...
uint32_t
MEMORY_CONTROLLER::get_occupancy(uint8_t queue_type, uint64_t address) {
...
uint32_t
MEMORY_CONTROLLER::get_size(uint8_t queue_type, uint64_t address) {
  uint32_t channel = dram_get_channel(address);
  if (queue_type == 1)
...
void
MEMORY_CONTROLLER::increment_WQ_FULL(uint64_t address) {
  uint32_t channel = dram_get_channel(address);
  WQ[channel].FULL++;
}
