...
void
record_roi_stats(uint32_t cpu, CACHE* cache) {
...
void
print_roi_stats(uint32_t cpu, CACHE* cache) {
...
void
print_sim_stats(uint32_t cpu, CACHE* cache) {
  uint64_t TOTAL_ACCESS = 0, TOTAL_HIT = 0, TOTAL_MISS = 0;
...
void
reset_cache_stats(uint32_t cpu, CACHE* cache) {
  for (uint32_t i = 0; i < NUM_TYPES; i++) {
    cache->ACCESS[i] = 0;
    cache->HIT[i] = 0;
    cache->MISS[i] = 0;
...
uint64_t
rotl64(uint64_t n, unsigned int c) {
...
main(int argc, char** argv) {
  ...
  printf(
      "Off-chip DRAM Size: %u MB Channels: %u Width: %u-bit Data Rate: %u MT/s\n",
      DRAM_SIZE,
      DRAM_CHANNELS,
      8 * DRAM_CHANNEL_WIDTH,
      DRAM_MTPS);
