...
void
O3_CPU::initialize_core() {}

void
O3_CPU::handle_branch() {
  ...
  while (continue_reading) {
    ...
    if (phasesim::Options::knob_cloudsuite) {
      ...
      if (!fread(&current_instr, instr_size, 1, trace_file)) {
        ...
        arch_instr.is_branch = current_instr.branch_info & BRANCH::branch;
        arch_instr.branch_taken = current_instr.branch_info & BRANCH::taken;
...
uint32_t
O3_CPU::add_to_rob(ooo_model_instr* arch_instr) {
...
uint32_t
O3_CPU::check_rob(uint64_t instr_id) {
...
O3_CPU::fetch_instruction() {
  ...
  uint32_t read_index =
      (ROB.last_read == (ROB.SIZE - 1)) ? 0 : (ROB.last_read + 1);
  ...
  uint32_t fetch_index =
      (ROB.last_fetch == (ROB.SIZE - 1)) ? 0 : (ROB.last_fetch + 1);
...
void
O3_CPU::do_scheduling(uint32_t rob_index) {
...
void
O3_CPU::reg_dependency(uint32_t rob_index) {
...
void
O3_CPU::reg_RAW_dependency(
    uint32_t prior, uint32_t current, uint32_t source_index) {
  for (uint32_t i = 0; i < MAX_INSTR_DESTINATIONS; i++) {
...
void
O3_CPU::schedule_memory_instruction() {
  if ((ROB.head == ROB.tail) && ROB.occupancy == 0)
    return;
...
void
O3_CPU::execute_memory_instruction() {
  operate_lsq();
  operate_cache();
}

void
O3_CPU::do_memory_scheduling(uint32_t rob_index) {
...
uint32_t
O3_CPU::check_and_add_lsq(uint32_t rob_index) {
...
void
O3_CPU::add_load_queue(uint32_t rob_index, uint32_t data_index) {
  ...
  if (LQ.entry[lq_index].virtual_address &&
      (LQ.entry[lq_index].producer_id == UINT64_MAX)) { // not released and no
                                                        // forwarding
...
void
O3_CPU::mem_RAW_dependency(
    uint32_t prior, uint32_t current, uint32_t data_index, uint32_t lq_index) {
  for (uint32_t i = 0; i < MAX_INSTR_DESTINATIONS; i++) {
    if (ROB.entry[prior].destination_memory[i] == 0)
      continue;

    if (ROB.entry[prior].destination_memory[i] ==
        ROB.entry[current].source_memory[data_index]) { //  store-to-load
                                                        //  forwarding check
...
void
O3_CPU::add_store_queue(uint32_t rob_index, uint32_t data_index) {
...
O3_CPU::operate_lsq() {
  ...
  while (load_issued < LQ_WIDTH) {
    if (RTL1[RTL1_head] < LQ_SIZE) {
      ...
      if (LQ.entry[lq_index].event_cycle <= current_core_cycle[cpu]) {
        int rq_index = execute_load(
            LQ.entry[lq_index].rob_index,
            lq_index,
...
void
O3_CPU::execute_store(
    uint32_t rob_index, uint32_t sq_index, uint32_t data_index) {
  ...
  if (ROB.entry[rob_index].is_producer) {
    ITERATE_SET(
        dependent, ROB.entry[rob_index].memory_instrs_depend_on_me, ROB_SIZE) {
      ...
      for (uint32_t j = 0; j < NUM_INSTR_SOURCES;
        ...
        if (ROB.entry[dependent].source_memory[j] &&
          ...
          if (ROB.entry[dependent].source_memory[j] ==
              SQ.entry[sq_index].virtual_address) { // this is required since a
...
int
O3_CPU::execute_load(
    uint32_t rob_index, uint32_t lq_index, uint32_t data_index) {
  // add it to L1D
  PACKET data_packet;
  data_packet.fill_level = FILL_L1;
  data_packet.cpu = cpu;
...
void
O3_CPU::complete_execution(uint32_t rob_index) {
...
void
O3_CPU::reg_RAW_release(uint32_t rob_index) {
...
void
O3_CPU::complete_instr_fetch(PACKET_QUEUE* queue, uint8_t is_it_tlb) {
  ...
  if (is_it_tlb) {
    ...
    ROB.entry[rob_index].instruction_pa =
        ...
        (ROB.entry[rob_index].ip & ((1 << LOG2_PAGE_SIZE) - 1)); // translated
                                                                 // address
  } else
    ...
    ITERATE_SET(i, queue->entry[index].rob_index_depend_on_me, ROB_SIZE) {
      ...
      if (is_it_tlb) {
        ...
        ROB.entry[i].instruction_pa =
            (queue->entry[index].instruction_pa << LOG2_PAGE_SIZE) |
            (ROB.entry[i].ip & ((1 << LOG2_PAGE_SIZE) - 1)); // translated
                                                             // address
      } else
...
void
O3_CPU::complete_data_fetch(PACKET_QUEUE* queue, uint8_t is_it_tlb) {
  uint32_t index = queue->head, rob_index = queue->entry[index].rob_index,
...
void
O3_CPU::handle_o3_fetch(PACKET* current_packet, uint32_t cache_type) {
...
void
O3_CPU::handle_merged_translation(PACKET* provider) {
...
void
O3_CPU::handle_merged_load(PACKET* provider) {
  ITERATE_SET(merged, provider->lq_index_depend_on_me, LQ.SIZE) {
...
void
O3_CPU::release_load_queue(uint32_t lq_index) {
  // release LQ entries
  DP(if (warmup_complete[cpu]) {
    cout << "[LQ] " << __func__ << " instr_id: " << LQ.entry[lq_index].instr_id
