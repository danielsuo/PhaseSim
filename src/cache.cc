  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
    ...
    if (do_fill) {
      ...
      // else if (cache_type == IS_L1D) {
          ...
          (cache_type == IS_L1D) && (MSHR.entry[mshr_index].type != PREFETCH)) {
        if (PROCESSED.occupancy < PROCESSED.SIZE)
          PROCESSED.add_queue(&MSHR.entry[mshr_index]);
      }
...
uint32_t
CACHE::get_set(uint64_t address) {
  return (uint32_t)(address & ((1 << lg2(NUM_SET)) - 1));
}

uint32_t
CACHE::get_way(uint64_t address, uint32_t set) {
...
void
CACHE::fill_cache(uint32_t set, uint32_t way, PACKET* packet) {
#ifdef SANITY_CHECK
  if (cache_type == IS_ITLB) {
    if (packet->data == 0)
      assert(0);
...
int
CACHE::invalidate_entry(uint64_t inval_addr) {
...
void
CACHE::return_data(PACKET* packet) {
...
int
CACHE::check_mshr(PACKET* packet) {
  // search mshr
...
uint32_t
CACHE::get_occupancy(uint8_t queue_type, uint64_t address) {
  if (queue_type == 0)
...
uint32_t
CACHE::get_size(uint8_t queue_type, uint64_t address) {
  if (queue_type == 0)
    return MSHR.SIZE;
  else if (queue_type == 1)
    return RQ.SIZE;
