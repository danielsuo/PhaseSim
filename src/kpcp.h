...
unsigned int
get_new_signature(unsigned int old_signature, int curr_delta) {
  if (curr_delta == 0)
...
int
L2_ST_update(uint32_t cpu, uint64_t addr) {
    ...
    if (table[match].valid && (table[match].tag == tag)) { // Hit
      ...
      if (table[match].signature == 0) { // First hit in L2_ST
        ...
        if (warmup_complete[cpu])
          L2_PF_DEBUG(printf(
              "ST_hit_first cpu: %d cl_addr: %lx page: %lx block: %d init_sig: %x delta: %d\n",
              cpu,
        ...
        if (warmup_complete[cpu])
          L2_PF_DEBUG(printf(
              "ST_hit cpu: %d cl_addr: %lx page: %lx block: %d old_sig: %x delta: %d\n",
              cpu,
              addr >> LOG2_BLOCK_SIZE,
              curr_page,
              curr_block,
  ...
  if (match == L2_ST_WAY) { // Miss
    ...
    for (int i = 0; i < L2_GHR_TRACK; i++) {
      ...
      if ((spec_block == curr_block) && (ghr_max <= L2_GHR[cpu][i].path_conf)) {
        ...
        ghr_idx = i;
        spec_sig = get_new_signature(
            L2_GHR[cpu][i].signature, L2_GHR[cpu][i].oop_delta);
        if (warmup_complete[cpu])
          L2_PF_DEBUG(printf(
              "cpu: %d OOP_match  L2_GHR[%d]  signature: %x  path_conf: %d  last_block: %d  oop_delta: %d  spec_block: %d == curr_block: %d  spec_sig: %x\n",
              cpu,
              i,
              L2_GHR[cpu][i].signature,
              L2_GHR[cpu][i].path_conf,
              L2_GHR[cpu][i].last_block,
    ...
    if (ghr_idx >= 0) {
      ...
      spec_sig = get_new_signature(
          L2_GHR[cpu][ghr_idx].signature, L2_GHR[cpu][ghr_idx].oop_delta);

      hit = 1;
      table[match].signature = spec_sig;
      if (warmup_complete[cpu])
        L2_PF_DEBUG(printf(
            "cpu: %d spec_update  page: %x  sig: %3x  delta: %3d  curr_block: %2d  last_block[NA]: %2d\n",
            cpu,
            tag,
            spec_sig,
            L2_GHR[cpu][ghr_idx].oop_delta,
            curr_block,
            L2_GHR[cpu][ghr_idx].last_block));
    }
#endif
  }
...
int
L2_ST_check(uint32_t cpu, uint64_t addr) {
  uint64_t curr_page = addr >> LOG2_PAGE_SIZE;
  ...
  for (match = 0; match < L2_ST_WAY; match++) {
    if (table[match].valid && (table[match].tag == tag)) {
      if (warmup_complete[cpu])
        L2_PF_DEBUG(printf(
            "ST_check found cpu: %d cl_addr: %lx page: %lx block: %ld old_sig: %x last_block: %d\n",
            cpu,
            addr >> LOG2_BLOCK_SIZE,
            curr_page,
            (addr >> LOG2_BLOCK_SIZE) & 0x3F,
            table[match].signature,
            table[match].last_block));
      return match;
    }
  }
...
void
L2_PT_update(uint32_t cpu, int signature, int delta) {
...
// Check sampler
void
notify_sampler(uint32_t cpu, int64_t address, int dirty, int useful) {
  /*
